      ...
      <div className="max-w-lg mx-auto w-full flex flex-col h-full px-4 pt-4 pb-20">
        ...
        <div className="relative mb-3">
          ...
          <input
            ...
            autoFocus
            className="w-full bg-slate-800 rounded-lg pl-9 pr-3 py-2.5 text-sm text-white outline-none focus:ring-2 focus:ring-teal-500"
          />
        </div>
        ...
        <div className="flex gap-1.5 overflow-x-auto pb-2 mb-2 no-scrollbar">
          <button
            ...
            className={`shrink-0 px-3 py-1 rounded-full text-xs font-medium transition ${
              filter === "All" ? "bg-teal-600 text-white" : "bg-slate-800 text-slate-400"
            }`}
          >
            All
          </button>
          {MUSCLE_GROUPS.map((mg) => (
            <button
              ...
              className={`shrink-0 px-3 py-1 rounded-full text-xs font-medium transition ${
                filter === mg ? "bg-teal-600 text-white" : "bg-slate-800 text-slate-400"
              }`}
            >
              {mg}
            </button>
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((ex) => (
              <button
                ...
                className="w-full flex items-center justify-between bg-slate-800 hover:bg-slate-700 rounded-xl px-4 py-3 transition text-left"
              >
                <div>
